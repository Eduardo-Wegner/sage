...
class get_data{
    ...
    extract_data($){
        return new Promise((resolve, reject) => {
            ...
            console.log(req_time.tz)
            $('.Box article.Box-row')
                .get()
                // eslint-disable-next-line complexity
                .map((repo, index) => {
                    const $repo = $(repo),
                    title = $repo.find('.h3').text().trim(),
                    [username, repoName] = title.split('/').map(v => v.trim()),
                    relativeUrl = this._github + $repo.find('.h3').find('a').attr('href'),
                    currentPeriodStarsString = $repo.find('.float-sm-right').text().trim() || '',
